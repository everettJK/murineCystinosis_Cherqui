...
intSites <- unlist(GRangesList(lapply(split(subjects, subjects$organism), function(x){
   ...
   intSites <- intSites[width(intSites) >= 5]
   
   # Setup parallelization.
   cluster <- parallel::makeCluster(CPUs)
   
   # Standardize fragment start positions.
   intSites <- gt23::standardizeSeqRanges.malani(intSites, standardizeStart=TRUE, standardizeEnd=FALSE)
   ...
   intSites <- unlist(GRangesList(parLapply(cluster, split(intSites, intSites$s), function(p){
     ...
     p <- gt23::nearestGenomicFeature(p, genome = p$genome[1])
     p <- p[gtools::mixedorder(p$posid),]
     ...
     # Nearest oncogene
     o <- gt23::nearestGenomicFeature(p, genome = p$genome[1], geneList = geneList)
     o <- o[gtools::mixedorder(o$posid),]
...
createUCSCintSiteAbundTrack(o$posid, o$estAbund, subject = 'CYS_human', title = 'CYS_human', outputFile = 'UCSC_CYS_human.ucsc')
system(paste0('scp UCSC_CYS_human.ucsc  microb120:/usr/share/nginx/html/UCSC/cherqui/'))
file.remove('UCSC_CYS_human.ucsc')


o <- subset(intSites, organism == 'mouse')
createUCSCintSiteAbundTrack(o$posid, o$estAbund, subject = 'CYS_mouse', title = 'CYS_mouse', outputFile = 'UCSC_CYS_mouse.ucsc')
system(paste0('scp UCSC_CYS_mouse.ucsc  microb120:/usr/share/nginx/html/UCSC/cherqui/'))
file.remove('UCSC_CYS_mouse.ucsc')
